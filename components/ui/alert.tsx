  ...
  ],
  {
    variants: {
      type: {
        ...
        banner:
          "grid grid-cols-[auto_1fr] gap-x-1.5 gap-y-1 rounded-lg border-0 p-3 has-[>[data-slot=alert-action]]:pr-9 [&_[data-slot=alert-button]>button[data-variant=secondary]]:bg-surface! [&_[data-slot=alert-button]>button[data-variant=secondary]]:shadow-elevation-sm [&_[data-slot=alert-button]>button[data-variant=secondary]]:hover:bg-gray-50/10! [&:not(:has(>[data-slot=alert-button]))]:flex! [&:not(:has(>[data-slot=alert-button]))]:items-center! [&:not(:has(>[data-slot=alert-button]))]:py-1.5! [&:not(:has(>[data-slot=alert-button]))]:pr-2! [&:not(:has(>[data-slot=alert-button]))]:pl-3.5! [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:relative! [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:top-auto! [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:right-auto! [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:ml-auto! [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:flex [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:items-center [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-action]]:gap-1 [&:not(:has(>[data-slot=alert-button]))>[data-slot=alert-title]]:text-normal! has-[>[data-slot=alert-button]]:[&>[data-slot=alert-button]]:col-span-2 has-[>svg]:[&>[data-slot=alert-description]]:col-span-2 has-[>svg]:[&>[data-slot=alert-title]]:col-start-2",
      },
      variant: {
