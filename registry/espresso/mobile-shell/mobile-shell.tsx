...
// A fixed-height header (--mobile-header-height, 52px) so a long title can
...
function MobileNav({ className, ...props }: React.ComponentProps<"nav">) {
  return (
    <nav
      ...
      className={cn(
        "grid h-16 shrink-0 auto-cols-fr grid-flow-col border-t border-border-soft bg-card [@media(display-mode:standalone)]:h-[calc(--spacing(16)+--spacing(4))] [@media(display-mode:standalone)]:pb-4",
        className
      )}
      {...props}
    />
...
function MobileNavItem({
  ...
  const cls = cn(
    "flex min-h-14 flex-col items-center justify-center text-muted-foreground transition outline-none select-none active:scale-95 [&_svg]:size-6 [&_svg]:shrink-0 data-[active=true]:text-foreground",
    className
  )
