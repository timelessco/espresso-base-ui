...
> {
  ...
  modifiers?: Modifiers
  collisionDetection?: React.ComponentProps<
    typeof DndContext
  >["collisionDetection"]
}
...
function Sortable<T>({
  ...
  accessibility,
  modifiers,
  // closestCenter over the rectIntersection default: with large items the
  // whole-rect overlap misses displaced neighbors (downward drops resolve
  // to null), while closest-center stays stable for lists and grids alike
  collisionDetection = closestCenter,
  children,
  ...props
}: SortableRootProps<T>) {
  const [activeId, setActiveId] = useState<UniqueIdentifier | null>(null)
  // dnd-kit derives its aria-describedby ids from a module counter that
  // drifts between server and client renders; a React.useId-backed id keeps
  // hydration stable
